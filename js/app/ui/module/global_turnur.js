define([
     ...
     * @type {TurnurModule}
     */
    let TurnurModule = class TurnurModule extends BaseModule {
        constructor(config = {}) {
        ...
        initTheraTable(){
            ...
            this._tableApi = $(tableEl).DataTable( {
                ...
                language: {
                    emptyTable: 'No Turnur connections found'
                },
                columnDefs: [
                    {
                        targets: 0,
                        ...
                        createdCell: function(cell, cellData, rowData, rowIndex, colIndex){
                            ...
                            cell.addEventListener('click', e => {
                                ...
                                if(rowData.connectionId){
                                    ...
                                    let systemDataThera = MapUtil.getSystemData(module._mapId, TurnurModule.systemIdThera, 'systemId');
                ...
                initComplete: function(settings, json){
                    module.getTurnurConnectionsData('callbackUpdateTableRows');
         ...
         * @param system
         */
        getTurnurConnectionsData(callback, system = null){

            this.getLocalStore().getItem('eveScoutTurnur')
                .then(cacheEntry => new Promise(resolve => {

                    if(
                        cacheEntry && cacheEntry.tSet &&
                        TurnurModule.ttlEveScoutResponse > BaseModule.now() - cacheEntry.tSet
                    ){
                        $(this.moduleElement).hideLoadingAnimation();
                        resolve(cacheEntry.value);
                    }else{
                        this.request('GET', 'SystemTurnur', [], {}, this, context => {
                            $(this.moduleElement).hideLoadingAnimation();
                        }).then(payload => {
                            let cacheEntry = {
                                tSet: BaseModule.now(),
                            ...
                            this.getLocalStore().setItem('eveScoutTurnur', cacheEntry)
                                .then(cacheEntry => resolve(cacheEntry.value));
                        }).catch(payload => {
                            let reason = payload.data.status + ' ' + payload.data.error;
                            this.showNotify({
                                title: payload.data.jqXHR.status + ': Turnur connections data',
                                text: reason,
                                type: 'warning'
                            });
                            resolve([]);
        ...
        callbackUpdateTableRows(connectionsData = [], systemThera = null){
            ...
            if(map && currentMapData){
                ...
                if(systemThera){
                    ...
                    let systemDataThera = MapUtil.getSystemData(this._mapId, TurnurModule.systemIdThera, 'systemId');
                    if(systemDataThera){
                        systemTheraFound = true;
                ...
                for(let currentConnectionData of Object.values(currentConnectionsData)){
                    ...
                    // swap source <-> target. 'Turnur' should be the source system
                    // -> In order to match the eve-scout response format for connections
                    if(target.name.toLowerCase() === 'turnur'){
                ...
                if(hadData && notification.length){
                    this.showNotify({title: 'Turnur connections updated', text: notification, type: 'success', textTrusted: true});
                }
            }else{
        ...
        update(mapId){
            return super.update(mapId).then(mapId => new Promise(resolve => {
                this.getTurnurConnectionsData('callbackUpdateTableRows');
    ...
    TurnurModule.isPlugin = false;                                   // module is defined as 'plugin'
    TurnurModule.scope = 'global';                                   // module scope controls how module gets updated and what type of data is injected
    TurnurModule.sortArea = 'b';                                     // default sortable area
    TurnurModule.position = 4;                                       // default sort/order position within sortable area
    TurnurModule.label = 'Turnur';                                   // static module label (e.g. description)
    TurnurModule.systemIdThera = 30002086;
    TurnurModule.ttlEveScoutResponse = 120;

    TurnurModule.defaultConfig = {
        className: 'pf-global-turnur-module',                        // class for module
        sortTargetAreas: ['a', 'b', 'c'],                            // sortable areas where module can be dragged into
        headline: 'Turnur Connection',

        // Turnur module
        theraTableId: 'pf-turnur-table-',                           // id prefix for all tables in module
